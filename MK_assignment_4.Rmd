...
author: |
  | Mikołaj Kompf
  | m.kompf@students.uu.nl
  | http://www.github.com/MikolajKompf
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
The presented mean for both groups is significantly different from each other, as the "growth" variable has a mean of 2.46. In comparison, "rgdp" variable has a much more significant mean of 5283.32. In addition the median also varies greatly, from 2.29 to 5393.00.
...
The purpose of adding the variable "rgdp60" is to adjust for the effect of the initial level of real GDP per capita in 1960. The "rgdp60" variable showcases the real GDP in 1960, and - by adding it as a control variable - we are able to analyse the relationship between the "treat" variable and economic growth. 
...
For us to understand the main driver of economic growth, we have to analyse the R-squared values in the model summaries. The R-squared value shows us the proportion of the variance in the dependent variable (growth) that can be explained by the independent variable in the model. 

By comparing the R-Squared values of the models, the highest R-Squared that suggests that independent variables are the main drivers of economic growth. Consequently, we know that the highest R-squared values in the model determine economic growth.  
...
list(model1,model2,model3,model4)|>
  modelsummary(gof_map=c("nobs","r.squared"),
               starts=T, title="regression table",
